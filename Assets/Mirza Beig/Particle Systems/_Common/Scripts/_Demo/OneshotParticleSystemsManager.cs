                ...
                public void InstantiateParticlePrefab(Vector2 mousePosition, float maxDistance)
                {
                    if (spawnedPrefabs != null)
                    {
                        if (!disableSpawn)
                        {
                            ...
                            Vector3 worldMousePosition = UnityEngine.Camera.main.ScreenToWorldPoint(position);

                            Vector3 directionToWorldMouse = worldMousePosition - UnityEngine.Camera.main.transform.position;
                            ...
                            Physics.Raycast(UnityEngine.Camera.main.transform.position + UnityEngine.Camera.main.transform.forward * 0.01f, directionToWorldMouse, out rayHit, maxDistance);

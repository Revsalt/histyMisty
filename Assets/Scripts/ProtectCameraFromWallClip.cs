...
using UnityEngine;
    ...
    // time taken to move when avoiding cliping (low value = fast, which it should be)
        public Vector3 offset;              // time taken to move when avoiding cliping (low value = fast, which it should be)
        ...
        private void LateUpdate()
        {
            ...
            for (int i = 0; i < cols.Length; i++)
            {
                if ((!cols[i].isTrigger) &&
                    ...
                    && !(cols[i].GetComponent<CharacterController>() != null
                    && cols[i].GetComponent<CharacterController>().CompareTag(dontClipTag))))
                {
            ...
            for (int i = 0; i < m_Hits.Length; i++)
            {
                ...
                if (m_Hits[i].distance < nearest && (!m_Hits[i].collider.isTrigger) &&
                    !(m_Hits[i].collider.attachedRigidbody != null &&
                      ...
                      && !(m_Hits[i].collider.GetComponent<CharacterController>() != null
                      && m_Hits[i].collider.GetComponent<CharacterController>().CompareTag(dontClipTag)))
                {
